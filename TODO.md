...
- **成果物**:
  - [x] インクリメンタル解析エンジン（`lexer.IncrementalLexer`統合・差分適用）
  - [x] エラー位置特定（UTF-16準拠のLSP座標、構文/検証エラーの精密マッピング）
  - [x] パフォーマンス最適化（トークン/AST/診断キャッシュ、診断の重複抑止）
- **実装内容**:
  - `didOpen`/`didChange`で差分適用しトークン再利用。変更範囲のみ再字句解析
  - `publishDiagnostics`のバージョン付与と重複通知抑止（キャッシュ）
  - UTF-16↔byteオフセット変換ユーティリティで位置精度を担保
  - ワークスペース初期スキャン（`.oriz`）でオープン外ファイルの参照/リネーム基盤を構築
- **依存関係**: 4.1.1
- **推定工数**: 大（30日）

- #### 4.1.3 コード補完システム ✅ 完了
- [x] **目的**: コンテキスト・スコープ・型ベースの補完（AI非依存）
- **成果物**:
  - [x] コンテキスト解析（カーソル前方の識別子プレフィックス抽出・スコープ感度）
  - [x] 補完候補生成（言語キーワード＋文書内識別子＋ワークスペースシンボル）
  - [x] ランキング（頻度優先→辞書順、重複排除、最大件数制限）
- **実装内容**:
...
- [x] **目的**: 型安全な大規模コード変更支援
- **成果物**:
  - [x] シンボル参照追跡（文書内/クロスファイル、ワークスペース索引ベース）
  - [x] 依存関係解析（スコープ/宣言競合の安全側検出・同一行既存名の衝突回避）
  - [x] 自動変更提案（`quickfix`と`refactor.extract`の最小編集生成）
- **実装内容**:
  - `rename`でクロスファイル`WorkspaceEdit`生成（未オープンファイルはディスク読み）
  - 競合検出：スコープ内既存名・対象行の既存新名を検知し該当編集をスキップ
  - `refactor.extract`：宣言挿入＋選択範囲置換の2編集のみで安定反映
- **依存関係**: 4.1.3
- **推定工数**: 大（32日）

### 4.2 パッケージマネージャー

#### 4.2.1 パッケージ解決エンジン ✅ 完了
- [x] **目的**: 依存関係の高速で正確な解決
- **関連ドキュメント**: design.md「Package Manager」
- **成果物**:
  - [x] Go実装の解決器: `internal/packagemanager/resolver.go`
  - [x] バージョン制約ソルバー（SemVer、範囲・チルダ・キャレット対応、交差結合）
  - [x] バックトラッキング解決、循環依存検出、決定的順序
  - [x] 高位/低位優先解の選択（`PreferHigher`）と最大深さ（`MaxDepth`）
  - [x] 単体テスト: `internal/packagemanager/resolver_test.go`
- **依存関係**: 3.4.3
- **推定工数**: 大（25日）
...
- [x] **目的**: 単一障害点のない配布システム
- **成果物**:
  - [x] コンテンツアドレス指定（CID: SHA-256ベース、`ComputeCID`）
  - [x] 分散インメモリレジストリ（`internal/packagemanager/registry.go`）
  - [x] Peer間の双方向リンクとベストエフォート複製（`ConnectPeers`/`Publish`）
  - [x] SemVer制約による検索（`Find`）と列挙（`List`/`All`）
  - [x] マネージャ統合（`Manager.ResolveAndFetch`）で解決→取得の一貫動作
  - [x] テスト: `registry_inmemory_test.go`/`manager_test.go`
- **依存関係**: 4.2.1
- **推定工数**: 大（35日）
...
- [x] **目的**: 完全に再現可能なビルド環境
- **成果物**:
  - [x] ロックファイル生成: `GenerateLockfile`（決定的順序・Canonical JSON・CID/SHA256含有）
  - [x] チェックサム検証: `VerifyLockfile`（CIDフェッチ→SHA256一致・メタデータ整合）
  - [x] 復元: `ResolutionFromLock` で固定解へ復元
  - [x] Manager統合: 解決→ロック→検証の往復テスト
- **依存関係**: 4.2.2
- **推定工数**: 大（20日）
...
#### 4.3.1 並列ビルドエンジン ✅ 完了
- [x] **目的**: 最大限のCPU活用による高速ビルド
- **成果物**:
  - [x] 依存関係グラフ: `internal/build/plan.go`（ターゲット/検証/部分グラフ抽出/循環検出）
  - [x] 並列実行エンジン: `internal/build/executor.go`（Nワーカ・依存尊重・結果/統計収集）
  - [x] 簡易負荷分散: Weight優先投入・同時実行上限はCPU数ベース
  - [x] テスト: `internal/build/executor_test.go`（循環検出・基本DAG・失敗伝播）
- **依存関係**: 4.2.1
- **推定工数**: 大（28日）

#### 4.3.2 インクリメンタルビルド ✅ 完了
- [x] **目的**: 変更部分のみの効率的な再コンパイル
- **成果物**:
  - [x] 変更検出: `internal/build/incremental.go`（入力スナップショット・SHA256・ModTime・差分判定）
  - [x] 選択実行: `Executor.ExecuteSelected` により差分ターゲットのみの実行が可能
  - [x] テスト: `internal/build/incremental_test.go`
- **依存関係**: 4.3.1
- **推定工数**: 大（25日）

#### 4.3.3 クロスコンパイル支援
 - [x] **目的**: 複数プラットフォーム向けビルドの自動化
 - **成果物**:
   - [x] ターゲット管理: `Platform{GOOS, GOARCH}` と検証
   - [x] ツールチェーン抽象化: `GoToolchain.BuildPackage` によるコマンド仕様生成（環境変数/ldflags/追加フラグ）
   - [x] テスト: `internal/build/toolchain_test.go`
 - **依存関係**: 4.3.2
 - **推定工数**: 大（22日）
...
#### 4.3.4 ビルドキャッシュシステム
 - [x] **目的**: チーム間でのビルド成果物共有
 - **成果物**:
   - [x] メモリLRUキャッシュ: `internal/build/cache.go`（上限・ヒット/ミス・エビクション・統計）
   - [x] ファイルシステムキャッシュ: マニフェスト+gzipブロブ構成、原子的更新、整合性検査、統計
   - [x] テスト: `internal/build/cache_test.go`
 - **依存関係**: 4.3.3
 - **推定工数**: 大（20日）
