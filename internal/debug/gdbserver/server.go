...
type Server struct {
	...
	pc    uint64
	bp    map[uint64]bool
	// Extended breakpoint/watchpoint maps to acknowledge Z1..Z4 requests.
	// These are currently advisory in the pseudo-execution model but kept to
	// maintain compatibility with clients that issue hardware and watchpoint commands.
	hwbp   map[uint64]bool // Z1/z1 hardware breakpoints
	wwatch map[uint64]bool // Z2/z2 write watchpoints
	rwatch map[uint64]bool // Z3/z3 read watchpoints
	awatch map[uint64]bool // Z4/z4 access watchpoints
	mu     sync.Mutex
