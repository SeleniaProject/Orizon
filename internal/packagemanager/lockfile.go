...
func GenerateLockfile(ctx context.Context, reg Registry, res Resolution) (Lockfile, []byte, error) {
    ...
    for n := range res { names = append(names, string(n)) }
    sort.Strings(names)
    ...
    for _, ns := range names {
        ...
        if err != nil { return Lockfile{}, nil, err }
...
func VerifyLockfile(ctx context.Context, reg Registry, lock Lockfile) error {
    ...
    if !isSortedLock(lock) { return errors.New("lockfile not sorted by name") }
...
func ResolutionFromLock(lock Lockfile) Resolution {
    ...
    for _, e := range lock.Entries { out[e.Name] = e.Version }
    return out
}
