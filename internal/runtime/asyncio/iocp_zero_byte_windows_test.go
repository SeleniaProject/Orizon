...
func TestIOCP_ZeroByteRecvTriggersReadable(t *testing.T) {
    ...
    if err := p.Register(cli, []EventType{Readable}, func(ev Event){ if ev.Type==Readable { select{case got<-struct{}{}: default:} } }); err != nil { t.Fatal(err) }

    // Send a single byte to complete the pending zero-byte recv
    if _, err := srv.Write([]byte{0x42}); err != nil { t.Fatal(err) }
...
func TestIOCP_ZeroByteSendTriggersWritable(t *testing.T) {
    ...
    got := make(chan struct{}, 1)
    if err := p.Register(cli, []EventType{Writable}, func(ev Event){ if ev.Type==Writable { select{case got<-struct{}{}: default:} } }); err != nil { t.Fatal(err) }
