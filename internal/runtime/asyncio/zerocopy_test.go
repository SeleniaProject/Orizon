...
func TestZeroCopy_CopyConnToConn(t *testing.T) {
    ...
    defer cancel()
    if _, err := CopyConnToConn(ctx, client, srcConn); err != nil { t.Fatal(err) }

    buf := make([]byte, 8)
    if _, err := io.ReadFull(client, buf); err != nil { t.Fatal(err) }
    if string(buf) != "12345678" { t.Fatalf("unexpected echo: %q", string(buf)) }
}
...
func TestZeroCopy_CopyFileToConn(t *testing.T) {
    ...
    ctx, cancel := context.WithTimeout(context.Background(), time.Second)
    defer cancel()
    if _, err := CopyFileToConn(ctx, conn, f); err != nil { t.Fatal(err) }

    select {
    case s := <-recv:
        if s != "abcdef" { t.Fatalf("unexpected: %q", s) }
