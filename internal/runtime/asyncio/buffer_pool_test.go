...
func TestBytePool_GetPut(t *testing.T) {
    ...
    if cap(buf) < 1500 { t.Fatalf("cap too small: %d", cap(buf)) }
    sizes, inuse := bp.Stats()
    _ = sizes
    var sum int64
    for _, v := range inuse { sum += v }
    if sum == 0 { t.Fatal("expected inuse > 0") }
    bp.Put(buf)
}
...
func TestBytePool_Oversize(t *testing.T) {
    ...
    if cap(buf) < (1<<20) { t.Fatalf("cap too small: %d", cap(buf)) }
