...
func SpliceConnToConn(ctx context.Context, dst net.Conn, src net.Conn, length int64) (int64, error) {
    // get fds
    var sfd, dfd int
    if sc, ok := src.(interface{ SyscallConn() (syscall.RawConn, error) }); ok {
        rc, err := sc.SyscallConn(); if err != nil { return 0, err }
        _ = rc.Control(func(fd uintptr){ sfd = int(fd) })
    } else { return 0, io.ErrUnexpectedEOF }
    if sc, ok := dst.(interface{ SyscallConn() (syscall.RawConn, error) }); ok {
        rc, err := sc.SyscallConn(); if err != nil { return 0, err }
    ...
    // Create pipe
    p := make([]int, 2)
    if err := unix.Pipe(p); err != nil { return 0, err }
    pr, pw := p[0], p[1]
    defer unix.Close(pr); defer unix.Close(pw)
    ...
    for length <= 0 || transferred < length {
        ...
        if err != nil {
            if err == unix.EAGAIN || err == unix.EINTR { continue }
        ...
        for off < n1 {
            ...
            if err2 != nil {
                if err2 == unix.EAGAIN || err2 == unix.EINTR { continue }
