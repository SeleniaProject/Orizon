...
func TestWSAPoll_NoEventsAfterDeregister_UnderTraffic(t *testing.T) {
    ...
    var got int32
    if err := p.Register(cli, []EventType{Readable, Writable, Error}, func(ev Event){ atomic.AddInt32(&got,1) }); err != nil { t.Fatal(err) }
    ...
    if err := p.Deregister(cli); err != nil { t.Fatal(err) }
