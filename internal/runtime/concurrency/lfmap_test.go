...
func TestLockFreeMap_String_Basic(t *testing.T) {
    ...
    if _, ok := m.Load("x"); ok { t.Fatal("unexpected present") }
    m.Store("x", 10)
    if v, ok := m.Load("x"); !ok || v != 10 { t.Fatalf("got %v %v", v, ok) }
    if v, existed := m.LoadOrStore("x", 20); !existed || v != 10 { t.Fatalf("loadorstore: %v %v", v, existed) }
    if !m.Delete("x") { t.Fatal("delete failed") }
    if _, ok := m.Load("x"); ok { t.Fatal("still present after delete") }
}
