...
func (rs *RemoteSystem) Start(nodeName, addr string) error {
	...
	if err := rs.Trans.Start(addr, handler); err != nil {
		return err
	}
	...
	if rs.Discover != nil {
		_ = rs.Discover.Register(nodeName, rs.Address)
	}

	if rs.RetryMaxAttempts == 0 {
		rs.RetryMaxAttempts = 6
	}

	if rs.RetryInitialMs == 0 {
		rs.RetryInitialMs = 50
	}

	if rs.RetryMaxBackoffMs == 0 {
		rs.RetryMaxBackoffMs = 800
	}
...
func (rs *RemoteSystem) Stop() error {
	...
	rs.started = false
	if rs.Discover != nil {
		rs.Discover.Unregister(rs.NodeName)
	}

	if rs.Trans != nil {
		return rs.Trans.Stop()
	}
...
func (rs *RemoteSystem) Send(remoteAddrOrNode, receiverName string, msgType uint32, payload interface{}) error {
	...
	if rs.Discover != nil {
		if addr, ok := rs.Discover.Resolve(remoteAddrOrNode); ok {
			target = addr
		}
	}
...
func (rs *RemoteSystem) sendWithRetry(target string, env Envelope) error {
	...
	for i := 0; i < attempts; i++ {
		...
		if rs.Discover != nil {
			if addr, ok := rs.Discover.Resolve(env.ReceiverNode); ok {
				target = addr
			}
		}
...
func (rs *RemoteSystem) SendWithRetry(remoteAddrOrNode, receiverName string, msgType uint32, payload interface{}, attempts int, baseDelayMs int) error {
	...
	if attempts > 0 {
		rs.RetryMaxAttempts = attempts
	}

	if baseDelayMs > 0 {
		rs.RetryInitialMs = baseDelayMs
	}
...
func (rs *RemoteSystem) receive(env Envelope) error {
	...
	if rs.Resolver == nil || rs.Local == nil {
		return fmt.Errorf("remote not bound to local system")
	}
