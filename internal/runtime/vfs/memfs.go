...
func (f *memFile) Read(p []byte) (int, error)        { f.mu.RLock(); defer f.mu.RUnlock(); return f.buf.Read(p) }
func (f *memFile) Write(p []byte) (int, error)       { f.mu.Lock(); defer f.mu.Unlock(); n, err := f.wbuf.Write(p); f.buf = bytes.NewReader(f.wbuf.Bytes()); f.mod = time.Now(); return n, err }
func (f *memFile) ReadAt(p []byte, off int64) (int, error) { f.mu.RLock(); defer f.mu.RUnlock(); return f.buf.ReadAt(p, off) }
...
func (f *memFile) Seek(off int64, whence int) (int64, error) { f.mu.Lock(); defer f.mu.Unlock(); return f.buf.Seek(off, whence) }
func (f *memFile) Close() error                               { return nil }
func (f *memFile) Sync() error                                { return nil }
func (f *memFile) Stat() (fs.FileInfo, error)                 { return fileInfo{name: path.Base(f.name), size: int64(f.buf.Len()), mode: f.mode, mod: f.mod}, nil }
...
func (m *MemFS) Walk(root string, fn func(fullPath string, d fs.DirEntry, err error) error) error {
    if fn == nil { return errors.New("nil walk fn") }
    entries, _ := m.ReadDir(root)
    for _, de := range entries {
        p := path.Join(root, de.Name())
        if err := fn(p, de, nil); err != nil { return err }
        if de.IsDir() {
            if err := m.Walk(p, fn); err != nil { return err }
        }
    }
    return nil
}
