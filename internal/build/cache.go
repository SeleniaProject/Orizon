...
func (c *InMemoryLRUCache) moveToFront(n *lruNode) {
    if c.llHead == n { return }
    // detach
    if n.prev != nil { n.prev.next = n.next }
    if n.next != nil { n.next.prev = n.prev }
    if c.llTail == n { c.llTail = n.prev }
    // insert at head
    n.prev = nil
    n.next = c.llHead
    if c.llHead != nil { c.llHead.prev = n }
    c.llHead = n
    if c.llTail == nil { c.llTail = n }
}

func (c *InMemoryLRUCache) evictIfNeeded() {
    for len(c.table) > c.capacity {
        // evict tail
        if c.llTail == nil { return }
        n := c.llTail
        delete(c.table, n.key)
        if n.prev != nil { n.prev.next = nil }
        c.llTail = n.prev
        if c.llTail == nil { c.llHead = nil }
        c.stats.Evictions++
...
func artifactSize(a Artifact) int64 {
    var s int64
    for _, b := range a.Files { s += int64(len(b)) }
    return s
}
...
func (fc *FSCache) pathForBlobs(key CacheKey) string { return filepath.Join(fc.pathForKey(key), "blobs") }
func (fc *FSCache) pathForManifest(key CacheKey) string { return filepath.Join(fc.pathForKey(key), "manifest.json") }

func (fc *FSCache) Get(key CacheKey) (Artifact, bool, error) {
    fc.mu.Lock(); defer fc.mu.Unlock()
    manPath := fc.pathForManifest(key)
    b, err := os.ReadFile(manPath)
    if err != nil {
        if errors.Is(err, os.ErrNotExist) { fc.stats.Misses++; return Artifact{}, false, nil }
        return Artifact{}, false, err
    }
    var man fsManifest
    if err := json.Unmarshal(b, &man); err != nil { return Artifact{}, false, err }
    ...
    for _, fe := range man.Files {
        ...
        if err != nil { return Artifact{}, false, err }
        var data []byte
        if fe.Compressed {
        ...
        if int64(len(data)) != fe.Size { return Artifact{}, false, fmt.Errorf("size mismatch for %s", fe.Name) }
...
func (b bytesReader) Read(p []byte) (int, error) { n := copy(p, b); if n < len(b) { return n, nil }; return n, io.EOF }

func (fc *FSCache) Put(key CacheKey, a Artifact) error {
    fc.mu.Lock(); defer fc.mu.Unlock()
    dir := fc.pathForKey(key)
    if err := os.MkdirAll(fc.pathForBlobs(key), 0o755); err != nil { return err }
    ...
    for name, data := range a.Files {
        ...
        gw := gzip.NewWriter(f)
        if _, err := gw.Write(data); err != nil { gw.Close(); f.Close(); return err }
        if err := gw.Close(); err != nil { f.Close(); return err }
    ...
    if err := os.WriteFile(tmp, mb, 0o644); err != nil { return err }
    if err := os.Rename(tmp, fc.pathForManifest(key)); err != nil { return err }
    fc.stats.Entries++
