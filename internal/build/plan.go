...
// The context may be canceled for fail-fast or deadline exceeded.
...
func (p *Plan) AddTarget(t Target) error {
	...
	if t.Action == nil {
		return errors.New("target action is nil")
	}

	if _, ok := p.nodes[t.ID]; ok {
		return fmt.Errorf("duplicate target: %s", t.ID)
	}
...
func (p *Plan) All() []Target {
	...
	for _, s := range ids {
		out = append(out, *p.nodes[TargetID(s)])
	}
...
func (p *Plan) Validate() error {
	...
	for _, t := range p.nodes {
		for _, d := range t.Deps {
			if _, ok := p.nodes[d]; !ok {
				return fmt.Errorf("%s depends on missing %s", t.ID, d)
			}
		}
	}
	...
	for _, s := range ids {
		if color[TargetID(s)] == white {
			if err := visit(TargetID(s)); err != nil {
				return err
			}
		}
	}
