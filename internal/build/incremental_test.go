...
func writeTempFile(t *testing.T, dir, name, contents string) string {
    ...
    if err := os.WriteFile(p, []byte(contents), 0o644); err != nil { t.Fatalf("write: %v", err) }
    return p
}
...
func TestIncremental_SnapshotAndDiff(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(dirty) != 0 { t.Fatalf("expected no dirty, got %v", dirty) }

    // Change file
    time.Sleep(10 * time.Millisecond)
    if err := os.WriteFile(a, []byte("HELLO"), 0o644); err != nil { t.Fatal(err) }
    ...
    if err != nil { t.Fatal(err) }
    if len(dirty) != 1 || dirty[0] != "T1" { t.Fatalf("expected T1 dirty, got %v", dirty) }
    ...
    if err != nil { t.Fatal(err) }
    if len(dirty) != 1 || dirty[0] != "T1" { t.Fatalf("expected T1 dirty on add, got %v", dirty) }

    // Remove one file
    if err := os.Remove(b); err != nil { t.Fatal(err) }
    ...
    if err != nil { t.Fatal(err) }
    if len(dirty) != 1 || dirty[0] != "T1" { t.Fatalf("expected T1 dirty on remove, got %v", dirty) }
}
