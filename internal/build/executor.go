...
func (e *Executor) Execute(ctx context.Context, plan *Plan, roots []TargetID) ([]Result, Stats, error) {
    ...
    if err := sub.Validate(); err != nil { return nil, Stats{}, err }
    ...
    type item struct { id TargetID; weight int }
    ready := make([]item, 0)
    for id, t := range sub.nodes { if indeg[id] == 0 { ready = append(ready, item{id: id, weight: t.Weight}) } }
    // stable order for determinism
    sort.Slice(ready, func(i, j int) bool { if ready[i].weight == ready[j].weight { return ready[i].id < ready[j].id }; return ready[i].weight > ready[j].weight })
    ...
    for {
        ...
        if int(stats.Succeeded+stats.Failed) == len(sub.nodes) { break }
        // backpressure stats
        if cur := atomic.LoadInt64(&running); cur > stats.MaxParallel { stats.MaxParallel = cur }
    }
    close(jobs)
