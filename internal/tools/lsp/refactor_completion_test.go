...
func TestCompletionResolveAndExtractRefactor(t *testing.T) {
    ...
    if err != nil { t.Fatalf("completion: %v", err) }
    res := comp["result"].(map[string]any)
    items := res["items"].([]any)
    if len(items) == 0 { t.Fatalf("no completion items") }
    ...
    if err != nil { t.Fatalf("codeAction: %v", err) }
    if acts["result"] == nil { t.Fatalf("no actions") }
}
