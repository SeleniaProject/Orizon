...
type FileTokenCache struct {
	// Content fingerprint for change detection using SHA-256
	// Alternative considered: CRC32 (faster but higher collision probability)
	ContentHash [32]byte

	// Cached tokens with position information for binary search
	Tokens []CachedToken

	// Line start positions for efficient line-based invalidation
	// Critical for typical editing patterns (single-line changes)
	LineStarts []int
...
type CachedToken struct {
	...
	// Absolute byte positions in the source file
	// Using int instead of int32 for compatibility with large files (>2GB)
	AbsoluteStart int
	AbsoluteEnd   int

	// Line and column information for editor integration
	Line   int
	Column int

	// Dependencies tracking for context-sensitive tokens
	// Example: String interpolation, nested comments
...
type Change struct {
	// Position information using absolute byte offsets
	Start int
	...
	// Change metadata for optimization
	Type   ChangeType
	Line   int
	Column int
}
...
type InvalidRange struct {
	Start     int     // Absolute byte position
	End       int     // Absolute byte position (-1 for end of file)
	ChangeRef *Change // Reference to the change that caused invalidation
}
