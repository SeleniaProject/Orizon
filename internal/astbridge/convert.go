...
func FromParserProgram(src *p.Program) (*ast.Program, error) {
    ...
    for _, d := range src.Declarations {
        ...
        if conv != nil { dst.Declarations = append(dst.Declarations, conv) }
    }
    return dst, nil
}
...
func fromParserDecl(d p.Declaration) (ast.Declaration, error) {
    ...
    case *p.ExpressionStatement:
        ...
        if err != nil { return nil, err }
        if decl, ok := s.(ast.Declaration); ok { return decl, nil }
...
func fromParserFunc(fn *p.FunctionDeclaration) (*ast.FunctionDeclaration, error) {
    ...
    var ret ast.Type
    var err error
    if fn.ReturnType != nil { ret, err = fromParserType(fn.ReturnType); if err != nil { return nil, err } }
...
func toParserFunc(fn *ast.FunctionDeclaration) (*p.FunctionDeclaration, error) {
    ...
    var ret p.Type
    var err error
    if fn.ReturnType != nil { ret, err = toParserType(fn.ReturnType); if err != nil { return nil, err } }
    bodyStmt, err := toParserStmt(fn.Body)
    if err != nil { return nil, err }
    blk, _ := bodyStmt.(*p.BlockStatement)
    if blk == nil { blk = &p.BlockStatement{} }
...
func fromParserVar(v *p.VariableDeclaration) (*ast.VariableDeclaration, error) {
    var t ast.Type
    var err error
    if v.TypeSpec != nil { t, err = fromParserType(v.TypeSpec); if err != nil { return nil, err } }
    val, err := fromParserExpr(v.Initializer)
    if err != nil { return nil, err }
    kind := ast.VarKindLet
    if v.IsMutable { kind = ast.VarKindVar }
...
func toParserVar(v *ast.VariableDeclaration) (*p.VariableDeclaration, error) {
    var t p.Type
    var err error
    if v.Type != nil { t, err = toParserType(v.Type); if err != nil { return nil, err } }
...
func fromParserStmt(s p.Statement) (ast.Statement, error) {
    ...
    case *p.ReturnStatement:
        ...
        var err error
        if n.Value != nil { val, err = fromParserExpr(n.Value); if err != nil { return nil, err } }
    ...
    case *p.IfStatement:
        ...
        if n.ElseStmt != nil { elseStmt, err = fromParserStmt(n.ElseStmt); if err != nil { return nil, err } }
...
func toParserStmt(s ast.Statement) (p.Statement, error) {
    ...
    case *ast.ReturnStatement:
        ...
        if n.Value != nil { val = toParserExprOrNil(n.Value) }
        return &p.ReturnStatement{Span: toParserSpan(n.Span), Value: val}, nil
    case *ast.IfStatement:
        var elseStmt p.Statement
        var err error
        if n.ElseBlock != nil { elseStmt, err = toParserStmt(n.ElseBlock); if err != nil { return nil, err } }
...
func ensureBlock(s ast.Statement) *ast.BlockStatement {
    if b, ok := s.(*ast.BlockStatement); ok { return b }
...
func fromParserExpr(e p.Expression) (ast.Expression, error) {
    ...
    case *p.BinaryExpression:
        left, err := fromParserExpr(n.Left); if err != nil { return nil, err }
        right, err := fromParserExpr(n.Right); if err != nil { return nil, err }
        op, err := fromParserOperator(n.Operator); if err != nil { return nil, err }
        return &ast.BinaryExpression{Span: fromParserSpan(n.Span), Left: left, Operator: op, Right: right}, nil
    case *p.UnaryExpression:
        operand, err := fromParserExpr(n.Operand); if err != nil { return nil, err }
        op, err := fromParserOperator(n.Operator); if err != nil { return nil, err }
        return &ast.UnaryExpression{Span: fromParserSpan(n.Span), Operator: op, Operand: operand}, nil
    case *p.CallExpression:
        fn, err := fromParserExpr(n.Function); if err != nil { return nil, err }
        args := make([]ast.Expression, 0, len(n.Arguments))
        for _, a := range n.Arguments { ca, err := fromParserExpr(a); if err != nil { return nil, err }; args = append(args, ca) }
...
func toParserExprOrNil(e ast.Expression) p.Expression { if e == nil { return nil }; out, _ := toParserExpr(e); return out }
...
func toParserExpr(e ast.Expression) (p.Expression, error) {
    ...
    case *ast.BinaryExpression:
        l, err := toParserExpr(n.Left); if err != nil { return nil, err }
        r, err := toParserExpr(n.Right); if err != nil { return nil, err }
        return &p.BinaryExpression{Span: toParserSpan(n.Span), Left: l, Operator: toParserOperator(n.Operator), Right: r}, nil
    case *ast.UnaryExpression:
        o, err := toParserExpr(n.Operand); if err != nil { return nil, err }
        return &p.UnaryExpression{Span: toParserSpan(n.Span), Operator: toParserOperator(n.Operator), Operand: o}, nil
    case *ast.CallExpression:
        f, err := toParserExpr(n.Function); if err != nil { return nil, err }
        args := make([]p.Expression, 0, len(n.Arguments))
        for _, a := range n.Arguments { ca, err := toParserExpr(a); if err != nil { return nil, err }; args = append(args, ca) }
...
func toParserLiteral(l *ast.Literal) *p.Literal {
    ...
    case ast.LiteralFloat:
        if v, ok := l.Value.(float64); ok { return &p.Literal{Span: toParserSpan(l.Span), Value: v, Kind: p.LiteralFloat} }
        return &p.Literal{Span: toParserSpan(l.Span), Value: 0.0, Kind: p.LiteralFloat}
    case ast.LiteralString:
        if v, ok := l.Value.(string); ok { return &p.Literal{Span: toParserSpan(l.Span), Value: v, Kind: p.LiteralString} }
        return &p.Literal{Span: toParserSpan(l.Span), Value: "", Kind: p.LiteralString}
    case ast.LiteralBoolean:
        if v, ok := l.Value.(bool); ok { return &p.Literal{Span: toParserSpan(l.Span), Value: v, Kind: p.LiteralBool} }
...
func fromParserOperator(op *p.Operator) (ast.Operator, error) {
    switch op.Value {
    case "+": return ast.OpAdd, nil
    case "-": return ast.OpSub, nil
    case "*": return ast.OpMul, nil
    case "/": return ast.OpDiv, nil
    case "%": return ast.OpMod, nil
    case "==": return ast.OpEq, nil
    case "!=": return ast.OpNe, nil
    case "<": return ast.OpLt, nil
    case "<=": return ast.OpLe, nil
    case ">": return ast.OpGt, nil
    case ">=": return ast.OpGe, nil
    case "&&": return ast.OpAnd, nil
    case "||": return ast.OpOr, nil
    case "!": return ast.OpNot, nil
    default: return 0, fmt.Errorf("unsupported operator %q", op.Value)
    }
}

func toParserOperator(op ast.Operator) *p.Operator { return &p.Operator{Value: op.String(), Kind: p.BinaryOp} }

func mapBasicTypeName(name string) ast.BasicKind {
    switch name {
    case "int": return ast.BasicInt
    case "float": return ast.BasicFloat
    case "string": return ast.BasicString
    case "bool": return ast.BasicBool
    case "char": return ast.BasicChar
    case "void": return ast.BasicVoid
    default: return ast.BasicInt
    }
}

// ==== Span Conversion ====

func fromParserSpan(s p.Span) position.Span { return position.Span{ Start: position.Position{Filename: s.Start.File, Line: s.Start.Line, Column: s.Start.Column, Offset: s.Start.Offset}, End: position.Position{Filename: s.End.File, Line: s.End.Line, Column: s.End.Column, Offset: s.End.Offset} } }
func toParserSpan(s position.Span) p.Span { return p.Span{ Start: p.Position{File: s.Start.Filename, Line: s.Start.Line, Column: s.Start.Column, Offset: s.Start.Offset}, End: p.Position{File: s.End.Filename, Line: s.End.Line, Column: s.End.Column, Offset: s.End.Offset} } }
