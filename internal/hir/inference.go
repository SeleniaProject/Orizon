...
type TypeInferenceEngine struct {
	typeVarCounter int                  // Counter for generating unique type variables
	constraints    []TypeConstraint     // Collected type constraints
	substitutions  *Substitution        // Current substitution map
	environment    *TypeEnvironment     // Type environment stack
	errors         []TypeInferenceError // Collected inference errors
}

// TypeVariable represents a type variable in the inference system
type TypeVariable struct {
	ID    int      // Unique identifier for this type variable
	Name  string   // Human-readable name (e.g., 'a, 'b, 't1, 't2)
	Kind  TypeKind // Kind of the type variable (for kind checking)
	Level int      // Quantification level for let-polymorphism
	Bound bool     // Whether this variable is bound to a concrete type
}

// TypeScheme represents a polymorphic type scheme (∀α.τ)
type TypeScheme struct {
	Variables []TypeVariable // Quantified type variables
	Type      TypeInfo       // The type with possible free variables
}
...
func (engine *TypeInferenceEngine) unifyTuple(tuple1, tuple2 TypeInfo) error {
	...
	for i := 0; i < len(tuple1.Parameters); i++ {
		...
		if err != nil {
			return fmt.Errorf("tuple element %d unification failed: %v", i, err)
		}
	}
...
func (engine *TypeInferenceEngine) unifyStruct(struct1, struct2 TypeInfo) error {
	...
	for i, field1 := range struct1.Fields {
		...
		if err != nil {
			return fmt.Errorf("struct field %s unification failed: %v", field1.Name, err)
		}
	}
