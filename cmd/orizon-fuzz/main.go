...
func main() {
	...
	if corpusDir != "" {
		...
		for _, e := range entries {
			...
			b, err := os.ReadFile(corpusDir + string(os.PathSeparator) + e.Name())
			if err == nil && len(b) > 0 {
		...
		wrapped = func(data []byte) error {
			...
			if corpusOut != "" {
				...
				if grew {
					...
					path := corpusOut + string(os.PathSeparator) + hexname
	...
	if per > 0 || minOnCrash {
		...
		effective = func(data []byte) error {
			...
			if err != nil && minOnCrash {
				...
				_ = os.WriteFile(minDir+string(os.PathSeparator)+name, min, 0o644)
			}
			return err
		}
	}
	...
	for {
		...
		if len(parts) >= 2 {
			...
			if dec, err := hex.DecodeString(raw); err == nil && len(dec) > 0 {
				...
				_ = os.WriteFile(w.dir+string(os.PathSeparator)+name, dec, 0o644)
			}
		}
	}
	return len(p), nil
